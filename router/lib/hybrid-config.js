...
function applyUserConfig(CFG, user) {
  ...
  if (user.local && typeof user.local === 'object') {
    ...
    if (Object.prototype.hasOwnProperty.call(L, 'fast_model')) {
      CFG.local.fast_model = typeof L.fast_model === 'string' ? L.fast_model.trim() : '';
    }
  }
...
function localModelUnsetInConfigFile(routerDir) {
  ...
  const L = user && user.local && typeof user.local === 'object' ? user.local : null;
  if (!L) return true;
...
function loadAndApply(CFG, routerDir) {
  ...
  let fileHost = null;
  if (user && user.listen && typeof user.listen.host === 'string' && user.listen.host.trim()) {
...
function saveLocalRoutingSettings(routerDir, { models, smart_routing, fast_model } = {}) {
  const p = configPath(routerDir);
  let obj = {};
  if (fs.existsSync(p)) {
    try {
