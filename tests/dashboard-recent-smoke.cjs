...
const TEST_PORT = Number.parseInt(
  process.env.TEST_ROUTER_PORT_SMOKE || "20938",
  10,
);
...
test("dashboard HTML + system-stats + pool routing POST (no admin token)", async (t) => {
  ...
  await waitFor(async () => {
    const r = await httpRequest(
      "GET",
      `http://127.0.0.1:${TEST_PORT}/api/health`,
    );
    return r.status === 200;
  });
  ...
  for (const needle of [
    ...
    assert.ok(
      ...
      `dashboard HTML should include: ${needle}`,
    );
  }
  assert.ok(
  ...
  const headerUi = await httpRequest(
    "GET",
    `http://127.0.0.1:${TEST_PORT}/header-ui`,
  );
  assert.strictEqual(headerUi.status, 200);
  assert.ok(
    headerUi.body.includes("header-ui-preview-note"),
    "header-ui preview banner present",
  );

  const stats = await httpRequest(
    "GET",
    `http://127.0.0.1:${TEST_PORT}/api/system-stats`,
  );
  assert.strictEqual(stats.status, 200);
  const sj = JSON.parse(stats.body);
  assert.ok("cpu" in sj && "ram" in sj);

  const rawGlobal = await httpRequest(
    "GET",
    `http://127.0.0.1:${TEST_PORT}/api/router/model-params-raw?which=global`,
  );
